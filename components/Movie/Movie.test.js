...
test("calls the onToggleLike handler function with the id when the like button is clicked", async () => {
  const user = userEvent.setup();
  const onToggleLike = jest.fn();
  render(
    <Movie id="1337" name="The Matrix Reloaded" onToggleLike={onToggleLike} />
  );
  const matrixLikeButton = screen.getByRole("button", {
    name: "like The Matrix Reloaded",
  });
  await user.click(matrixLikeButton);
  expect(onToggleLike).toHaveBeenCalledWith("1337");
});

test("calls the onDeleteMovie handler function with the id when the delete button is clicked", async () => {
  const user = userEvent.setup();
